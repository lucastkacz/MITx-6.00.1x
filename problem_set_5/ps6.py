    ...
    def build_shift_dict(self, shift):
        ...
        def _shift(letters):
            ...
            for letter in letters:
                ...
                new_letter_order = letter_order + shift
                if new_letter_order <= ord(letters[-1]):
                    shifted_letters += chr(new_letter_order)
                else:
                    new_letter_order = new_letter_order - len(letters)
                    shifted_letters += chr(new_letter_order)
                    
            return shifted_letters
                    
        shifted_lower_letters = _shift(LOWERCASELETTERS)
        shifted_upper_letters = _shift(UPPERCASELETTERS)

        shifted_dict  = dict(zip(LOWERCASELETTERS + UPPERCASELETTERS, shifted_lower_letters + shifted_upper_letters))
        return shifted_dict
